...
<style>
    ...
    .input-section h3 { color: #a78bfa; font-size: 1rem; margin-bottom: 16px; font-family: 'Space Grotesk', sans-serif; }
...
<div class="page-header">
    <h1>🧠 Test Neural Network Model</h1>
    <p>Fill in student information below and click Predict to get a Pass/Fail result from the Neural Network</p>
</div>
""", unsafe_allow_html=True)
...
try:
    ...
    st.error(f"❌ Could not load model files. Make sure model2_nn.h5, scaler.pkl, and label_encoders.pkl are in the same folder as app.py.\n\nError: {e}")
    st.stop()
...
if st.button("🔮 Predict with Neural Network", use_container_width=True):
    ...
    # Build feature dict (same column order as training)
    features = {
    ...
    # Neural network outputs a probability between 0 and 1
    prob = float(nn_model.predict(X_input, verbose=0)[0][0])
    prediction = 1 if prob >= 0.5 else 0

    pass_prob = round(prob * 100, 1)
    fail_prob = round((1 - prob) * 100, 1)

    if prediction == 1:
        st.markdown(f"""
        <div class="result-pass">
            <div class="result-title" style="color:#34d399;">✅ PASS</div>
            <div class="result-subtitle">The Neural Network predicts this student will pass the exam</div>
            <br>
        ...
        <div class="result-fail">
            ...
            <div class="result-subtitle">The Neural Network predicts this student will not pass the exam</div>
            <br>
    ...
    # Show what the NN "thinks" internally
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f"""
    <div style="background:#1e293b; border:1px solid #334155; border-radius:12px; padding:20px; text-align:center;">
        <div style="color:#94a3b8; font-size:0.85rem;">Raw Neural Network Output (sigmoid value)</div>
        <div style="color:#a78bfa; font-size:2rem; font-weight:700; font-family:'Space Grotesk',sans-serif;">{prob:.4f}</div>
        <div style="color:#64748b; font-size:0.8rem;">Values closer to 1.0 = more confident Pass · Values closer to 0.0 = more confident Fail</div>
    </div>
    """, unsafe_allow_html=True)
